...
#
# Required variables:
#   OPENLDR_API_URL      — API base URL (default: https://api.openldr.org.mz)
#   OPENLDR_API_USER     — API username
#   OPENLDR_API_PASSWORD — API password
#
# Usage:
#   ./query-api.sh login                           # Get JWT token
#   ./query-api.sh get "/hiv/vl/summary/header_indicators/?interval_dates=2024-01-01,2024-12-31"
...
API_URL="${OPENLDR_API_URL:-https://api.openldr.org.mz}"
...
EXAMPLES:
  ...
  ./query-api.sh get "/hiv/vl/summary/suppression_by_province/?interval_dates=2024-01-01,2024-12-31&province=Gaza"

  # Get facilities (no auth needed)
  ./query-api.sh get-noauth "/dict/facilities/"

  # Get TB positivity by month for Maputo
  ./query-api.sh get "/tb/gx/summary/positivity_by_month/?interval_dates=2024-01-01,2024-12-31&province=Maputo"

  # EID tested samples for POC labs
  ./query-api.sh get "/hiv/eid/laboratories/tested_samples/?interval_dates=2024-01-01,2024-12-31&lab_type=poc"
HELP
}
...
check_auth_env() {
  ...
  if [ "$missing" -eq 1 ]; then
    ...
    echo "" >&2
    echo "  OPENLDR_API_URL=https://api.openldr.org.mz" >&2

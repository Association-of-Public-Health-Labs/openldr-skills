...
|-----------|--------|----------|---------|
| `interval_dates` | `YYYY-MM-DD,YYYY-MM-DD` | Recommended | `interval_dates=2024-01-01,2024-12-31` |
...
`interval_dates` is a special range parameter: pass both dates in one
comma-separated value. The deployed API splits this value into start and end
dates. Passing it as repeated keys can cause `list index out of range`.

Other multi-value filters use repeated keys:
```
CORRECT DATE RANGE: ?interval_dates=2024-01-01,2024-12-31
WRONG DATE RANGE:   ?interval_dates=2024-01-01&interval_dates=2024-12-31

CORRECT FILTER: ?province=Maputo&province=Gaza
WRONG FILTER:   ?province=Maputo,Gaza
```
...
```
OPENLDR_API_URL=https://api.openldr.org.mz
...
scripts/query-api.sh get "/hiv/vl/summary/header_indicators/?interval_dates=2024-01-01,2024-12-31"
...
| `list index out of range` | Date range was likely passed as repeated keys | Retry with `interval_dates=START_DATE,END_DATE` |
